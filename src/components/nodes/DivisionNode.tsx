...
type DivisionNodeProps = {
  data: NodeData;
};

function DivisionNode({ data }: DivisionNodeProps) {
  ...
  const calculateQuotient = () => {
    ...
    // Calculate quotient (first number divided by second number)
    if (inputValues.length === 0) return 0;
    if (inputValues.length === 1) return inputValues[0];
    if (inputValues[1] === 0) return 0; // Prevent division by zero
    return inputValues[0] / inputValues[1];
  };

  // Initial calculation
  useEffect(() => {
    const newResult = calculateQuotient();
    setResult(newResult);
  ...
  return (
    <div className="bg-background border-2 rounded-lg p-3 min-w-[150px]">
      <div className="font-semibold mb-2">Divide</div>
