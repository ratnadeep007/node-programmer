...
type MultiplicationNodeProps = {
  data: NodeData;
};

function MultiplicationNode({ data }: MultiplicationNodeProps) {
  ...
  const calculateProduct = () => {
    ...
    return inputValues.reduce((acc: number, val: number) => acc * val, 1);
  };

  // Initial calculation
  useEffect(() => {
    const newResult = calculateProduct();
    setResult(newResult);
  ...
  return (
    <div className="bg-background border-2 rounded-lg p-3 min-w-[150px]">
      <div className="font-semibold mb-2">Multiply</div>
